...
test('Can create a FactorNode from an object literal', function () {
  ...
  equal(fac.name, 'fac');
  return equal(fac.levels.toString(), [
    1,
    2,
    3,
    4,
...
test('can create a FactorSetNode from an object literal', function () {
  ...
  equal(fnode.factorNames.toString(), [
    'wordtype',
    'repnum',
    'lag'
  ].toString(), fnode.factorNames.toString());
  return equal(fnode.varmap.wordtype.levels.toString(), [
    'word',
    'pseudo'
  ].toString(), fnode.varmap.wordtype.levels.toString());
});
module('TaskNode');
test('TaskNode correctly extracts names of its inputs', function () {
  var fspec1, fspec2, fspec3, tspec;
  fspec1 = new Psy.FactorSpec(1, 1, 'a', [
    1,
    2,
    3
  ]);
  fspec2 = new Psy.FactorSpec(1, 1, 'b', [
    5,
    7,
    9,
    21
  ]);
  fspec3 = new Psy.FactorSpec(1, 1, 'dddd', [
    5,
    7,
    9,
    21
  ]);
  tspec = new Psy.TaskNode([
    fspec1,
    fspec2,
    fspec3
  ], []);
  equal([
    ...
    'b',
    'dddd'
  ].toString(), tspec.factorNames.toString());
  return equal(tspec.varmap.a.levels.toString(), [
    1,
    2,
    3
  ].toString());
...
test('can build an ItemNode from object literal', function () {
  ...
  equal(node.attributes.x.toString(), [
    1,
    2,
    3
  ].toString(), node.attributes.x.toString());
  return equal(node.attributes.y.toString(), [
    4,
    5,
    6
  ].toString(), node.attributes.x.toString());
...
test('AbsoluteLayout correcty converts percentage to fraction', function () {
  ...
  equal(xy[0], 1e3 * .1, '10% of 1000 is ' + xy[0]);
  return equal(xy[1], 1e3 * .9, '90% of 1000 is ' + xy[1]);
});
